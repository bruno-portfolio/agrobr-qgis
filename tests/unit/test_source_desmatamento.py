...
from tests.mocks.mock_agrobr import MockDesmatamento


@pytest.fixture(autouse=True)
def _register_desmatamento(monkeypatch: pytest.MonkeyPatch) -> None:
    mock_module = MagicMock()
    mock_desmatamento = MockDesmatamento()
    mock_module.desmatamento = mock_desmatamento
    monkeypatch.setitem(sys.modules, "agrobr", MagicMock())
    monkeypatch.setitem(sys.modules, "agrobr.sync", mock_module)
    monkeypatch.setitem(sys.modules, "agrobr.sync.desmatamento", mock_desmatamento)
    from agrobr_qgis.sources.desmatamento import DeterSource, ProdesSource

    SourceRegistry.register(DeterSource)
    SourceRegistry.register(ProdesSource)


class TestDeterSource:
    def test_registered_in_registry(self) -> None:
        assert SourceRegistry.get("deter") is not None

    def test_id(self) -> None:
        src = SourceRegistry.get("deter")
    ...
    def test_fetch_returns_dataframe(self) -> None:
        src_cls = SourceRegistry.get("deter")
...
class TestProdesSource:
    def test_registered_in_registry(self) -> None:
        assert SourceRegistry.get("prodes") is not None

    def test_id(self) -> None:
        src = SourceRegistry.get("prodes")
        assert src is not None
        assert src.id() == "prodes"
    ...
    def test_fetch_returns_dataframe(self) -> None:
        src_cls = SourceRegistry.get("prodes")
        assert src_cls is not None
        result = src_cls().fetch()
        assert isinstance(result, pd.DataFrame)
        assert len(result) == 10
